...
- **Navigation button placement**: a new Navigation Buttons row in the Sidebar section with a Set Position button. It opens an editor like the mascot one, floating the site's ↑ ↓ buttons above the overlay, with sliders for scale, horizontal and vertical offsets, spacing (up to 300px) and a Reverse Order switch; every slider has a typed number field beside it, and a typed value past the slider's range widens it. The buttons can also be dragged into place on the page while the editor is open. Save persists at once, Use Site Default clears it, Cancel and Escape restore the stored placement, and the placement is included in exports. The arrows use a tightly cropped icon so reversing the order leaves no hidden gap, and the editor is painted with the theme's colors
- **Settings apply live**: every control in the options panel takes effect as soon as it changes (typed sizes after a short pause), while only Save writes to storage. Cancel, Escape, the backdrop and the header toggle ask "Discard unsaved changes?" when something is unsaved and revert to the stored settings on confirmation; Save closes without asking. Theme tiles preview live and theme file imports show up at once. The Tower/changed pills now cover Navigation Buttons, Fit Expanded Images and Follow Cursor
- **Theme custom CSS is isolated** in its own style element right after the main sheet, so an unclosed brace no longer swallows every StyleTower rule after it (sidebar, quick reply, icons, catalog, options), and the theme's rules win ties. Custom Font names and theme background image URLs are escaped, so a quote or parenthesis can no longer end a rule early or drop the theme variables after it
- **Startup is resilient**: each feature runs guarded, so a site change that breaks one hook no longer takes the theme down with it. Saves that fail (quota, blocked storage) are reported by name instead of silently dropped
- **Upload and import fixes**: Auto-Convert Images passes non-image uploads through untouched and hands the original file back when a decode fails. Settings import brings custom themes (sanitized) and Hidden Themes over from StyleTower/StyleChan exports, checks theme indices against the imported list, skips junk values, and stores saved site settings where the Restore button reads them. Themes missing secondary colors (older StyleChan/OneeChan exports) derive them from related colors instead of painting black
- Fit Expanded Images works again (its rule depended on a 4chan-X class); Fit Post Menu's right margin applies; Follow Cursor targets the site's own hover image and its description says what it does; Delete Post removes the post (or its file block) on success and returns to the board index after deleting an OP
- Relative Post Dates is retired: the site's own "Show relative time" covers it, and an enabled setting migrates to the site's key
- Theme tiles: hiding the selected default theme moves the selection to the first visible theme (also when a stored selection points at a since-hidden theme at load); the inert SFW/NSFW labels are replaced by a Selected marker; a half-typed hex in the theme editor keeps the current color instead of flashing the page black; Escape closes an open theme or mascot editor first, then the settings panel
- Auto scroll counts real posts only, so an open TS hover preview or inlined quote no longer stops the next real post from scrolling; replaced WEBM/MP4 loops carry preload="none" so offscreen loops fetch nothing on load; comment drafts are keyed by board and thread, so the +50 view of a thread shares the draft of its full view; the quick reply no longer polls for TS's posting controls when TS is not installed; an auto-hidden header leaves no top padding
- Saving an option that only takes effect on the next load shows a toast naming it, with a Reload now link; arrow keys step UI Font Size and Backlink Font Size like Font Size; the Export button says the file includes the saved Holotower name, email and post password
- The theme converter on the GitHub Pages tools site keeps a StyleChan/StyleTower theme's header colors and carries the hover keys instead of overwriting them with fallbacks
- Housekeeping: dead CSS for classes nothing sets on Holotower, unused theme variables and icons, duplicate selector groups and dead script members are gone; the Chrome extension uses manifest v3 with trimmed permissions and the dead update plumbing is dropped; userscript grants match the calls actually made; ESLint is clean (with a parseInt radix rule); a jsdom test suite (`npm test`) covers the script, and CONTRIBUTING documents build, test and release
